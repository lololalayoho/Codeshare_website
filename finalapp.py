...
def whosolved():
	...
	if request.method == 'POST':
		...
		result = cursor.fetchall()
		if result:
			sql = "SELECT u.id, s.date, p.origin, p.no, p.title, u.name FROM USER u, SOLVE s, PROBLEM p WHERE s.origin = '%s' AND s.no = '%s' AND s.id = u.id AND s.origin = p.origin AND s.no = p.no"%(origin, no)
			cursor.execute(sql)
			list=[]
			for result in cursor.fetchall():
				list.append({
